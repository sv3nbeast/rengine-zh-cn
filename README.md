...
<p>使用高度可定制且功能强大的扫描模式，快速发现攻击面并识别漏洞。
使用reNgine的持续监控、更深入的侦察和开源的漏洞扫描器，您可以放心。</p>

<h4>What is reNgine?</h4>
<p align="left">reNgine是一个web应用程序侦察套件，专注于通过模式、侦察数据关联、持续监控、数据库支持的侦察数据以及简单直观的用户界面实现高度可配置的简化侦察过程。凭借子扫描、深层关联、报告生成等功能，reNgine旨在弥补传统侦察工具的不足，并可能是现有商业工具的更好替代品。
...
reNgine具有基于YAML的高度可配置扫描模式，允许渗透测试人员根据自己的选择创建任意多个侦察模式，根据自己的意愿进行配置，并针对扫描的任何目标使用这些模式。这些模式允许渗透测试人员使用他们选择的工具和他们选择的配置。reNgine自带多个扫描模式，如全扫描、被动扫描、截图收集、OSINT模式等。
